...
public class Handshake {
	 ...
	 * @param outputStream
	 *            Stream to which the certificate will be written
	 * @return
	 * @throws CertificateException
	 * @throws IOException
	 */
	static Certificate sendCertificate(String certificatePath, DataOutputStream outputStream)
	 ...
	 */
	static Certificate processCertificate(DataInputStream inputStream) throws CertificateException,
	...
	/**
	 * Generates a random 8 byte nonce. Encrypts the nonce using the provided public
	 * key, and writes the encrypted nonce to the provided output stream.
	 * 
	 * @param outputStream
	 *            - Stream to which the encrypted nonce will be written.
	 * @param publicKey
	 *            - Public key belonging to whomever this nonce is being sent. The
	 *            public key is used to encrypt the nonce.
	 ...
	 * @throws IOException
	 */
	static byte[] createAndSendNonce(DataOutputStream outputStream, PublicKey publicKey)
	 ...
	 * 
	 * @param inputStream
	 *            - Stream from which the encrypted nonce is read
	 * @param privateKey
	 *            - Key required to decrypt the nonce.
	 * @return
	 * @throws IOException
	 * @throws NoSuchAlgorithmException
	 * @throws NoSuchPaddingException
	 ...
	 * @throws BadPaddingException
	 */
	static byte[] receiveNonce(DataInputStream inputStream, PrivateKey privateKey)
	...
	/**
	 * Generates a Hashed Mutual-Authentication-Code. The hash function takes the
	 * following items in the following order, as is done by this particular SSL
	 * protocol: client's public key, server's public key, server's un-encrypted
	 * nonce, and the client's un-encrypted nonce. A string is also used at the end
	 * that represents the sender of the hash ("CLIENT" or "SERVER).
	 * 
	 * @param clientKey
	 * @param serverKey
	 * @param clientNonce
	 ...
	 * @throws NoSuchAlgorithmException
	 */
	static byte[] HMAC(PublicKey clientKey, PublicKey serverKey, byte[] clientNonce, byte[] serverNonce, String sender)
			throws NoSuchAlgorithmException {
	 ...
	 *            - For comparing incoming HMAC
	 * @param sender
	 *            - For comparing incoming HMAC, should be "SERVER" if incoming HMAC
	 *            was from the server, or should be "CLIENT" if the incoming HMAC is
	 *            from the client.
	 * @param inputStream
	 *            - Stream that the HMAC is being recieved on.
	 * @return
	 * @throws IOException
	 * @throws NoSuchAlgorithmException
	 */
	public static boolean verifyIncomingHMAC(PublicKey clientPublicKey, PublicKey serverPublicKey, byte[] clientNonce,
			byte[] serverNonce, String sender, DataInputStream inputStream)
	...
	/**
	 * Private helper method that takes an int and turns it into a byte array so
	 * that it can be used with a Cipher.Mac object
	 * 
	 * @param number
	 * @return
	 */
	static byte[] IntToByteArray(int number) {
		...
		// Mask part of int needed, shift all the way to right
		array[0] = (byte) ((number & 0xFF000000) >> 24);
		array[1] = (byte) ((number & 0x00FF0000) >> 16);
		array[2] = (byte) ((number & 0x0000FF00) >> 8);
		array[3] = (byte) ((number & 0x000000FF) >> 0);
